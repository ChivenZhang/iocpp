...
inline constexpr uint32_t iocpp_hash(const char* value)
{
...
inline constexpr uint32_t iocpp_hash(std::string const& value)
{
	return iocpp_hash(value.c_str());
}
...
public:
	...
	{
		auto name = ((uint64_t)iocpp_hash(typeid(T).name()) << 32) | N;
	...
	std::weak_ptr<std::shared_ptr<Bean>> create(uint32_t N = 0)
	{
		auto name1 = ((uint64_t)iocpp_hash(typeid(I).name()) << 32) | N;
		auto name2 = ((uint64_t)iocpp_hash(typeid(T).name()) << 32) | N;
		auto name3 = ((uint64_t)iocpp_hash(typeid(I).name()) << 32) | 0;
...
#define AUTOWIRE2(T, N) Autowire<T, iocpp_hash(N)>
// Autowire(Type or Interface of Type)
#define AUTOWIRE_THIS(T) AutowireThis<T>(0).bean()
// Autowire(Type or Interface of Type, Name)
#define AUTOWIRE2_THIS(T, N) AutowireThis<T>(iocpp_hash(N)).bean()

// Resource(Type, Name)
#define RESOURCE(T, N) Resource<T, T, iocpp_hash(N)>
// Resource(Type, Interface of Type, Name)
#define RESOURCE2(T, I, N) Resource<T, I, iocpp_hash(N)>
// Resource(Type, Name)
#define RESOURCE_THIS(T, N) ResourceThis<T, T>(iocpp_hash(N)).bean()
// Resource(Type, Interface of Type, Name)
#define RESOURCE2_THIS(T, I, N) ResourceThis<T, I>(iocpp_hash(N)).bean()